...
export class MessageGenerator {
  ...
  private selectRandomType(): MessageType {
    const types: MessageType[] = ['question', 'statement', 'command'];
   ...
   */
  private generateQuestion(constraints?: GenerationConstraints): string {
    const questions = [
      'What is your name?',
      'How are you today?',
   ...
   */
  private generateStatement(constraints?: GenerationConstraints): string {
    const statements = [
   ...
   */
  private generateCommand(constraints?: GenerationConstraints): string {
    const commands = [
      'Show me the options.',
   ...
   */
  private padMessage(message: string, minLength: number): string {
   ...
   */
  private randomLength(min: number, max: number): number {
   ...
   */
  generateVeryLongMessage(length: number = 10000): string {
   ...
   */
  private generateOpeningMessage(topic: string): string {
    const templates = [
   ...
   */
  private generateFollowUpMessage(topic: string, previousMessage: string, index: number): string {
